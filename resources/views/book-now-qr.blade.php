...
@section('extrajs')
    ...
    <script>
        ...
        async function displayIPInformation() {
            const ipInfo = await fetchIPInformation();
            if (ipInfo) {
                console.log(ipInfo, 'i m here');
                console.log('IP Information:', ipInfo);
                $.ajax({
                    url: "{{route('qr.data')}}", // URL where you want to send the POST request
                    type: 'POST', // HTTP method used
                    data: ipInfo, // Data object to be sent
        ...
        // Call displayIPInformation when the page loads
